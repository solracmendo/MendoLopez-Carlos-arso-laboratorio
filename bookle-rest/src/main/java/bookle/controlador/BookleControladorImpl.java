...
public class BookleControladorImpl implements BookleControlador {
	...
	public String createActividad(String titulo, String descripcion, String profesor, String email)
		...
		if(profesor == null || profesor.equals(""))
			throw new IllegalArgumentException("El profesor no puede ser nulo o vacio");
	...
	public void addDiaActividad(String id, Date fecha, int turnos) throws BookleException {
		...
		if (getDia(actividad, fecha) != null)
			throw new BookleException("La fecha establecida ya está en la agenda: " + fecha);
	...
	public int addTurnoActividad(String id, Date fecha) throws BookleException {
		...
		if (dia == null)
			throw new BookleException("La fecha establecida no está en la agenda: " + fecha);
	...
	public void removeTurnoActividad(String id, Date fecha, int turno) throws BookleException {
		...
		if (turno <= 0 || turno > turnos)
			throw new BookleException(
					"Índice de turno incorrecto: " + turno + " en la actividad " + id + " en la fecha " + fecha);
	...
	public boolean removeReserva(String id, String ticket) throws BookleException {
		...
		for (DiaActividad dia : actividad.getAgenda()) {
			...
			for (Turno turno : dia.getTurno()) {
				...
				if (turno.getReserva() != null && turno.getReserva().getId().equals(ticket)) {
